...
def init_two_layer_convnet(weight_scale=1e-3, bias_scale=0, input_shape=(3, 32, 32),
  ...
  model = {}
  n = filter_size * filter_size * C
  model['W1'] = weight_scale * np.random.randn(num_filters, C, filter_size, filter_size)
  #model['W1'] = 1/np.sqrt(n) * np.random.randn(num_filters, C, filter_size, filter_size)
  model['b1'] = bias_scale * np.random.randn(num_filters)
  
  n = num_filters * H * W / 4  
  model['W2'] = weight_scale * np.random.randn(num_filters * H * W / 4, num_classes)
  #model['W2'] = 1/np.sqrt(n) * np.random.randn(num_filters * H * W / 4, num_classes) * np.random.randn(num_filters * H * W / 4, num_classes)
...
def deep_convnet(X, model, y=None, reg=0.0):
  """
  Compute the loss and gradient for a deep ConvNet. The architecture
  is [conv-relu-pool]^n-affine-softmax, where the conv layer uses stride-1 "same"
  convolutions to preserve the input size; the pool layer uses non-overlapping
  2x2 pooling regions. We use L2 regularization on both the convolutional layer
  weights and the affine layer weights.
  ...
  scores, cache5 = affine_forward(a3, W5, b5)
  ...
  da2, dW3, db3 = conv_relu_pool_backward(da4, cache3)
  ...
  reg_loss = 0.5 * reg * sum(np.sum(W * W) for W in [W1, W2, W3, W5])

  loss = data_loss + reg_loss
  grads = {'W1': dW1, 'b1': db1, 'W2': dW2, 'b2': db2, 'W3': dW3, 'b3': db3, 'W5': dW5, 'b5': db5}
  
  return loss, grads

def init_deep_convnet(weight_scale=5e-2, bias_scale=0, input_shape=(3, 32, 32),
  ...
  # w = np.random.randn(n) * sqrt(2.0/n) with n number of inputs
  # dont use the weight scale for initialization but the method proposed in
  # lecture notes. Biasses initialized at 0.
  model = {}
  n = filter_size * filter_size * C
  model['W1'] = weight_scale * np.random.randn(num_filters, C, filter_size, filter_size)
  model['b1'] = bias_scale * np.random.randn(num_filters)
  ...
  #n = num_filters * H * W / 64
  #model['W4'] = weight_scale * np.random.randn(num_filters * H * W / 64, 256)
  #model['b4'] = bias_scale * np.random.randn(256)

  n = num_filters * H * W / 64
  model['W5'] = weight_scale * np.random.randn(num_filters * H * W / 64, num_classes)
...
def init_four_layer_convnet(input_shape=(3, 32, 32), num_classes=10,
                            filter_sizes=(3, 3), num_filters=(32, 32, 128),
                            weight_scale=1e-2, bias_scale=0, dtype=np.float32):
  """
  Initialize a four layer ConvNet with the following architecture:

  [conv - relu - pool]x2 - affine - relu - dropout - affine - softmax
  ...
  - num_classes: Number of classes over which classification will be performed.
    Default is 10 for CIFAR-10.
  - filter_sizes: The height and width of filters in the convolutional layers.
  - num_filters: Tuple (F1, F2, H) where F1 and F2 are the number of filters to use in the
    convolutional layers and H is the number of neurons to use in the hidden
    affine layer.
  - weight_scale: Weights are initialized from a gaussian distribution with
    standard deviation equal to weight_scale.
  - bias_scale: Biases are initialized from a gaussian distribution with
...
def four_layer_convnet(X, model, y=None, reg=0.0, dropout=None):
  """
  Compute the loss and gradient for a simple three layer ConvNet that uses
  the following architecture:
  ...
  - model: Dictionary mapping parameter names to values; it should contain
    the following parameters:
    - W1, b1, W2, b2: Weights and biases for convolutional layers
    - W2, b2, W3, b3: Weights and biases for affine layers
  - y: Integer array of shape (N,) giving the labels for the training samples
    in X. This is optional; if it is not given then return classification
    scores; if it is given then instead return loss and gradients.
  ...
  if dropout is None:
    ...
    da3 = dropout_backward(dd2, cache3)
...
def init_five_layer_convnet(input_shape=(3, 32, 32), num_classes=10,
                            filter_sizes=(3, 3, 3), num_filters=(32, 32, 64, 128),
                            weight_scale=5e-2, bias_scale=0, dtype=np.float32):
  """
...
def five_layer_convnet(X, model, y=None, reg=0.0, dropout=1.0,
  ...
  d4, cache5 = a4, cache4#dropout_forward(a4, dropout_param)
  ...
  da4 = dd4#dropout_backward(dd4, cache5)
  da3, dW4, db4 = affine_relu_backward(da4, cache4)
